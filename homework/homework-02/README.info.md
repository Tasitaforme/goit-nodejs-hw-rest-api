...
програми. Після підключається модуль у додатку і він додає змінні оточення в
об'єкт `process.env`, і вже звідти, не показуючи значення цих змінних, можемо
використовувати їх у додатку. Само собою файл `.env` потрібно додати до файлу
`.gitignore`.
